...
//   - For `.` form: ALWAYS safe to rewrite, even in chains. `a.b.c(x)` becomes
//     `a.b["c"](x)`, which is semantically identical.
//   - For `:` form: receiver must be a *plain variable*, not itself a field
//     access. We duplicate the receiver as the first arg, so `a.b:c(x)` would
//     incorrectly become `a.b["c"](b, x)` — `b` evaluated as a bare global.
//     The previous token before the receiver IDENT must NOT be `.` or `:`.
...
function rewriteMethodCalls(tokens) {
  ...
  for (let k = 0; k + 3 < sig.length; k++) {
    ...
    // For `:` rewrites only: the receiver must be a real variable, not a
    // field access. `remote.OnServerEvent:Connect(...)` MUST NOT be rewritten
    // because the receiver of :Connect is `remote.OnServerEvent` — emitting
    // just `OnServerEvent` as the first arg would be a bare global lookup
    // and evaluate to nil at runtime.
